library SetSudokuLib {
  ...
  function contains(Set storage set, uint key, bytes1 cellValue) public view returns(bytes1 result) {
    assembly {
      ...
      let val := mod(cellValue, 0xff) // bytes1
      let position := shl(mul(val, 8), val)
      position := and(position, mul(exp(0x100, val), 0xff)) // clean the value, ensure the right offset(in this casse) and size
  ...
  function insert(Set storage set, uint key, bytes32 action, bytes1 cellValue) external {
    ...
    assembly {
      ...
      let shifted := mod( cellValue, 0xff ) // 0x000... x
      let position := shl( mul( shifted, 8 ), shifted) // 8 bits .. 1 byte
      let result :=  and( mul( exp( 0x100, shifted ) , 0xff ) , position )
      let mask := not(mul(exp(0x0100, shifted), 0xff))// mask to clear offset for insert
      result := or(and(mask, sload(keccak256(0, 0x40))), result) // putting mask here results in dup operation
